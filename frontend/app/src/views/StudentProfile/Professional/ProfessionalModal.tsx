...
export const ProfessionalModal: React.FC<ProfessionalModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await dispatch(
        id
          ? actions.updateProfessionalQuote({
              content: value,
              id,
            })
          : actions.addProfessionalQuote({
              content: value,
              type: `PROFESSIONAL`,
            }),
      )
  ...
  return (
    <>
      <Modal
        title={t(`profile.student.professionalScreen.professional`)}
        subtitle={t(`profile.student.profileScreen.pleaseLimit`)}
        subtitleQuestion={t(`profile.student.professionalScreen.whatIndustry`)}
