...
export const Activism: React.FC<ServicesProps> = ({ service }) => {
  ...
  if (!service?.activism.length && !isViewingOwnProfile) {
    return null
  }
    ...
    service && (
      <div className="student-profile-tab-item">
        <div className="student-profile-tab-item-title">
          <div>{t(`profile.student.serviceScreen.activism`)}</div>

          {!!service?.activism?.length && isViewingOwnProfile && (
            <IconBox
              icon={Plus}
              variant={`dark`}
              onClick={() =>
                dispatch(actions.toggleAddActivityModal(ActivityType.ACTIVISM))
              }
              tag={`button`}
              shadow
            />
        ...
        <div className="service-activism">
          {!!service?.activism?.length &&
            service.activism.map((item) => {
              const reference =
                references?.find(
                ...
                <>
                  ...
                  {editActivityModalOpen === item.id ? (
                    <EditActivityModal
                      ...
                      title={t(`profile.student.serviceScreen.activism`)}
                    />
                  ) : null}
