...
interface GraduationRateProps {
  graduationRate: number | null | undefined
  onEdit?: () => void
}

const GraduationRate: React.VFC<GraduationRateProps> = ({
  graduationRate,
  onEdit,
}) => {
        ...
        datasets: [
          {
            data: [
              graduationRate / 100,
              graduationRate !== 100 ? 1 - graduationRate / 100 : null,
  ...
  return (
    <MDBCard>
      <MDBCardBody>
        <MDBCardTitle>
          {t(`profile.university.overviewScreen.graduationRate`)}
        ...
        <MDBCardText>
          <div className={Styles.container}>
            <canvas id={`graduationRateChart`} />

            <h3 className={Styles.label}>
              {Math.floor(graduationRate ?? 0).toFixed(0)}%
            </h3>
          </div>
        </MDBCardText>
      </MDBCardBody>
