...
interface SatSubjectsModalProps {
  isOpen: boolean
  toggle: () => void
  courses: Course[] | []
}

export const SatSubjectsModal: React.FC<SatSubjectsModalProps> = ({
  isOpen,
  toggle,
  courses,
}) => {
  ...
  useEffect(() => {
    ...
    if (!isEditSubjectCoursesModalOpen && isOpen) {
      setTimeout(
        () => (document.body.className = `modal-open overflow-hidden`),
        100,
      )
  ...
  return (
    <Modal
      title={t(`profile.student.transcriptScreen.yourSatSubjectTests`)}
      subtitle={t(
        `profile.student.transcriptScreen.yourSatSubjectTestsSubtitle`,
      )}
      isOpen={isOpen}
      toggle={() => {
        toggle()
      ...
      body={
        <div className="sat-subject-modal-body">
          <div className="sat-subject-modal-body-header">
            <div className="sat-subject-modal-body-header-item">
              {t(`profile.student.transcriptScreen.testName`)}
            </div>
            <div className="sat-subject-modal-body-header-grade">
              {t(`common:score`)}
            </div>
          </div>
          <DndProvider
          ...
          <div
            className="sat-subject-modal-body-footer"
            onClick={() => {
              dispatch(actions.toggleAddSubjectCoursesModal(true))
              toggle()
            }}
          >
            {t(`profile.student.transcriptScreen.addMoreSubjectTests`)}
          </div>
        </div>
      }
