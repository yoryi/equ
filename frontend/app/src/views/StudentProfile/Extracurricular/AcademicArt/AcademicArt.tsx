...
interface AcademicArtProps {
  extracurriculars: ProfileExtracurriculars | null
}

export const AcademicArt: React.FC<AcademicArtProps> = ({
  extracurriculars,
}) => {
  ...
  if (!extracurriculars?.academicAndArts.length && !isViewingOwnProfile) {
    return null
  }
    ...
    extracurriculars && (
      <div className="student-profile-tab-item">
        <div className="student-profile-tab-item-title">
          <div>
            {t(`profile.student.extracurricularsScreen.academicAndArts`)}
          </div>
          ...
          {isViewingOwnProfile && (
            ...
            >
              ...
              {!!extracurriculars.academicAndArts?.length && (
                <IconBox
                  className={`mx-1`}
                  icon={Plus}
                  variant={`dark`}
                  onClick={() =>
                    dispatch(
                      actions.toggleAddActivityModal(ActivityType.ACADEMIC_ART),
                    )
                  }
                  tag={`button`}
                  shadow
        ...
        {!!extracurriculars.academicAndArts?.length &&
          extracurriculars.academicAndArts.map((item) => {
            const reference =
              references?.find(
              ...
              <>
                ...
                {editActivityModalOpen === item.id ? (
                  <EditActivityModal
                    ...
                    title={t(
                      `profile.student.extracurricularsScreen.academicAndArts`,
                    )}
                  />
                ) : null}
        ...
        {!extracurriculars?.academicAndArts?.length && isViewingOwnProfile && (
          <EmptyExperience
            title={t(`profile.student.extracurricularsScreen.academicAndArts`)}
            toggleModal={() =>
              dispatch(
                actions.toggleAddActivityModal(ActivityType.ACADEMIC_ART),
              )
